...
export async function POST(req: Request) {
  ...
  if (!process.env.ANTHROPIC_API_KEY) {
    return NextResponse.json(
      {
        error:
          "Shopping assistant isn't configured on this server yet (missing ANTHROPIC_API_KEY).",
      },
      { status: 503 }
    );
  }

  const listingsBrief = typedListings
    .map(
      (l) =>
...
"matches" should be an array of 0-3 listing ids (as strings, exactly as given above) that best fit what they said. If nothing fits well, return an empty array.`,
    ...
    const matchedListings = typedListings.filter((l) =>
      (parsed.matches || []).includes(l.id)
    );
    ...
    return NextResponse.json(
      { error: "Couldn't reach the assistant right now." },
      { status: 502 }
    );
  }
  }
